 ...
 *
 * ai-elements' `ModelSelectorLogo` renders these as `<img src="https://
 * models.dev/logos/{provider}.svg">`, which tells models.dev which provider the
 * user picked on every turn. The API is built so pages never learn provider
 * identity; fetching it from a third party is worse than showing it. Paths are
 * vendored from models.dev (MIT) and use `currentColor`, so they inherit theme.
 */
...
type Mark = {
  ...
  box: string;
  paths: string[];
  /** Stroked marks need no fill and a stroke width. */
  stroke?: number;
};
...
/** Aliases so an OpenAI-compatible or gateway id still lands on a mark. */
...
/**
 * Resolve a provider slug from a model id like `anthropic/claude-haiku-4.5`.
...
export function ProviderLogo({
  ...
  const mark = key ? (MARKS[key] ?? MARKS[ALIASES[key] ?? ""]) : undefined;

  if (!mark) {
    return (
      <Cpu
        aria-label={provider ? `${provider} logo` : "provider"}
